...
class PageTest < Minitest::Test
  ...
  def test_attributes
    with_config do |config|
      ...
      assert_equal config.src_dir.join("foo.html.erb").to_s, source.path.to_s
    end
  end
  ...
  def test_relative_path_to_in_root
    with_config do |config|
      File.write(config.template_dir.join("layouts", "default.html.erb"), "")
      source = Yass::Source.new(config, config.src_dir.join("foo.html.erb"))
      page = Yass::Page.new(config, source)
  ...
  def test_render
    with_config do |config|
      template = Yass::ErbTemplate.compile("<h1><%= title %></h1>")
  ...
  def test_template
    with_config do |config|
      File.write(config.template_dir.join("title.html.erb"), "<h1><%= title %></h1>")
      source = Yass::Source.new(config, config.src_dir.join("foo.html"))
      page = Yass::Page.new(config, source)
