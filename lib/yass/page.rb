module YASS
  class Page < File
    ...
    def files(glob = "**/*.*")
      Dir[config.src.join(glob)]
        .map { |path| Pathname.new(path).relative_path_from config.src }
        .map { |path| File.new(config, relative_path_to(path)) }
    end

    def templates
      @templates ||= Dir[config.templates.join("**/*.erb")].each_with_object({}) do |path, acc|
        template = Pathname.new(path)
        key = template.relative_path_from(config.templates).to_s
